...
export default function Header() {
  ...
  const [isOpen, setIsOpen] = useState(false); // 🛠️ Manual state for instant close
  ...
  const scrollToSection = (sectionId: string) => {
    ...
    if (section) {
      const yOffset = -80; // 🛠️ Leaner offset for leaner header
      const y =
        section.getBoundingClientRect().top + window.pageYOffset + yOffset;

      setIsOpen(false); // 🛠️ Vanish the menu immediately
  ...
  return (
    ...
    >
      {/* 🛠️ HEIGHT: py-4 on mobile for that precision profile */}
      <div className="w-full max-w-7xl mx-auto flex items-center justify-between py-4 md:py-6 px-6 md:px-12">
        <Link href="/" className="flex items-center space-x-4 group z-20">
          <Logo
            ...
            {["services", "about", "contact"].map((item) => (
              <li key={item}>
                <a
                  ...
                  className="font-bold text-lg text-white/80 hover:text-white transition-colors capitalize"
                >
                  {item}
                </a>
              </li>
        ...
        <div className="flex items-center space-x-4 z-20">
          {/* 🛠️ BUTTON SYMMETRY: h-11 for the new leaner height */}
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            >
              {/* 🛠️ ATMOSPHERE: Services Section Orbs */}
              ...
              <nav className="flex flex-col space-y-10 text-center z-10">
                {["services", "about", "contact"].map((item) => (
                  <a
                    ...
                    /* 🛠️ TEXT SIZE: text-5xl for premium balance */

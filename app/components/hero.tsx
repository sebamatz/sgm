    ...
    <section className="relative h-screen w-full flex items-center overflow-hidden bg-[#050505] text-white">
      ...
      <div className="absolute inset-0 z-0">
        <Canvas
          ...
          onCreated={({ gl }) => {
            // Kill the deprecation warning by using the Timer API
            // We cast to any to handle the internal THREE property swap
            (gl as any).clock = new (THREE as any).Timer();
          }}
        >
        ...
        <div className="max-w-4xl mt-20">
          ...
          >
            <button className="group relative flex items-center justify-center gap-4 bg-white/5 hover:bg-white/10 border border-white/10 backdrop-blur-md text-white px-8 h-16 md:h-20 rounded-full text-lg font-semibold tracking-wide transition-all duration-500 hover:border-white/30 overflow-hidden">
              ...
              <div className="absolute inset-0 h-full w-full bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full group-hover:animate-[shimmer_1.5s_infinite]" />
            </button>
          </motion.div>
        </div>
      </div>

      {/* Premium Agency Grain Overlay */}
      <div
